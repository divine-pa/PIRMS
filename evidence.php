...
<body>
  ...
  <div class="container">
      <!-- All your main page content here -->


    <!-- Search Bar -->
    <div class="mb-4">
      <input type="text" id="evidenceSearchInput" class="form-control form-control-lg" placeholder="Search by evidence ID, type, case number, or status..." />
            ...
            <?php else: ?>
              <?php foreach ($evidenceList as $row): ?>
                <tr>
                  ...
                  <td><?php echo htmlspecialchars($row['evidence_type']); ?></td>

                  <td><a href="#" class="text-decoration-none"><?php echo htmlspecialchars($row['case_id']); ?></a></td>

                  <td>
                    <?php
                    $statusColor = 'bg-secondary';
                    if ($row['condition_status'] == 'Good') $statusColor = 'bg-success';
                    if ($row['condition_status'] == 'Damaged') $statusColor = 'bg-danger';
                    if ($row['condition_status'] == 'Fair') $statusColor = 'bg-warning text-dark';
                    ?>
                    <span class="badge <?php echo $statusColor; ?>"><?php echo htmlspecialchars($row['condition_status']); ?></span>
                  </td>
  ...
  <script>
    document.getElementById('evidenceSearchInput').addEventListener('input', function() {
