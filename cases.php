...
// Notice the backticks around `case` because it is a reserved word
$sql = "
  SELECT c.case_id, c.title, c.status, c.priority, c.date_started,
         c.progress_percent, o.name AS officer_name
  FROM `case` c
...
if ($search !== '') {
  $sql .= " AND (c.title LIKE :search OR c.description LIKE :search OR c.case_id LIKE :search) ";
  $params[':search'] = '%' . $search . '%';
}

$sql .= " ORDER BY c.date_started DESC";
...
foreach ($cases as $c) {
  if ($c['status'] === 'Ongoing') $ongoingCases++;
...
<body>
  ...
  <?php if ($showSuccess): ?>
    <div id="successNotification" class="alert alert-success position-fixed top-0 start-50 translate-middle-x mt-3" role="alert" style="z-index: 9999; width: 90%; max-width: 500px; animation: slideDown 0.4s ease-out;">
      <div class="d-flex align-items-center">
        ...
        <strong>Success!</strong> New case has been added to the system.
      </div>
    </div>
    <style>
      @keyframes slideDown {
  ...
  <div class="container mb-0">
    <h2 style="color: #081b34;">Case Management</h2>
    <p style="color:rgb(139, 140, 140);">View and manage all ongoing and closed investigation cases.</p>
    <button onclick="location.href='newcases.php'" class="btn text-white fw-bold px-4 py-2 d-flex align-items-center float-end" style="background:#081b34;">
      <span class="me-2 fs-5">+</span> Add New Case
    </button>
  </div>
  </div>
  ...
  <div class="container py-4">
    <div class="mb-3">
      <input type="text" id="caseSearchInput" class="form-control" placeholder="Search by case ID, title, or officer" />
    </div>

    <div class="card mb-4">
      <div class="card-body">
        <h5 class="card-title mb-4">Case Overview</h5>
        <div class="table-responsive">
          <table class="table align-middle mb-0">
            ...
            <tbody>
              <?php if (!$cases): ?>
                <tr>
                  <td colspan="8" class="text-center text-muted small py-3">No cases found.</td>
                </tr>
              <?php else: ?>
                <?php foreach ($cases as $c): ?>
                  <tr>
                    <td><?php echo htmlspecialchars($c['case_id']); ?></td>
                    <td><?php echo htmlspecialchars($c['title']); ?></td>
                    <td><?php echo htmlspecialchars($c['officer_name'] ?? 'Unassigned'); ?></td>
                    <td>
                      <?php
                      $status = $c['status'];
                      $cls = $status === 'Ongoing' ? 'bg-primary' : 'bg-success';
                      ?>
                      <span class="badge <?php echo $cls; ?>"><?php echo htmlspecialchars($status); ?></span>
                    </td>
                    <td>
                      <?php
                      $pri = $c['priority'];
                      $pcls = 'bg-secondary';
                      if ($pri === 'High') $pcls = 'bg-danger';
                      elseif ($pri === 'Medium') $pcls = 'bg-warning text-dark';
                      ?>
                      <span class="badge <?php echo $pcls; ?>"><?php echo htmlspecialchars($pri); ?></span>
                    </td>
                    <td><?php echo htmlspecialchars($c['date_started']); ?></td>
                    <td>
                      <div class="progress" style="height: 8px;">
                        <div class="progress-bar bg-warning" role="progressbar" style="width: <?php echo (int)$c['progress_percent']; ?>%;"></div>
                      </div>
                      <small class="text-muted"><?php echo (int)$c['progress_percent']; ?>%</small>
                    </td>
                    <td>
                      <button onclick="location.href='case_details.php?case_id=<?php echo urlencode($c['case_id']); ?>'" class="btn btn-outline-secondary btn-sm">View</button>
                    </td>
                  </tr>
                <?php endforeach; ?>
              <?php endif; ?>
    ...
    <div class="row g-3">
      <div class="col-md-4">
        <div class="card text-center">
          <div class="card-body">
            <div class="display-6 mb-2"><?php echo $totalCases; ?></div>
            <p class="mb-0 text-muted">Total Cases</p>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="card text-center">
          <div class="card-body">
            <div class="display-6 mb-2"><?php echo $ongoingCases; ?></div>
      ...
      <div class="col-md-4">
        <div class="card text-center">
          <div class="card-body">
            <div class="display-6 mb-2"><?php echo $closedCases; ?></div>
      ...
      <div class="row pb-4">
        ...
        <div class="col-md-4 mb-4 mb-md-0 text-md-center text-center">
          ...
          <div class="small text-white-50">
            <div class="mb-1">contact@pirms.com</div>
            <div>123, Lekki, Lagos</div>
          </div>
        </div>
...
<body>
  ...
  <div class="container mb-0">
    <div class="text-left mb-4 mt-5">
      <h2 style="color: #081b34;">Case Management</h2>
      <p style="color:rgb(139, 140, 140);">View and manage all ongoing and closed investigation cases.</p>
      <button onclick="location.href='newcases.php'" type="button" class="btn text-white fw-bold px-4 py-2 d-flex align-items-center float-end" style="background:#081b34;">
        <span class="me-2 fs-5">+</span> Add New Case
      </button>
    </div>
  </div>
                ...
                <?php foreach ($cases as $c): ?>
                  <tr>
                    ...
                    <td>
                      ...
                      $status = $c['status'];
                      $cls = 'bg-secondary';
                      if ($status === 'Ongoing') $cls = 'bg-primary';
                      elseif ($status === 'Closed') $cls = 'bg-success';
                      ?>
                    ...
                    <td>
                      ...
                      elseif ($pri === 'Medium') $pcls = 'bg-warning text-dark';
                      ?>
                      <span class="badge <?php echo $pcls; ?>">
                    ...
                    <td>
                      <div class="progress" style="height: 8px;">
                        <div class="progress-bar" role="progressbar"
                          style="width: <?php echo (int)$c['progress_percent']; ?>%;"></div>
                      </div>
                    ...
                    <td>
                      <button onclick="location.href='case_details.php?case_id=<?php echo urlencode($c['case_id']); ?>'"
      ...
      <div class="col-md-4">
        <div class="card text-center">
          <div class="card-body">
            ...
            <p class="mb-0" style="color:rgb(139, 140, 140);">Completed</p>
          </div>
        </div>
      </div>
    </div>
