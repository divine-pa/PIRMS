...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  // Generate case ID
  $case_id = 'CASE-' . date('Y') . '-' . str_pad(rand(1, 999), 3, '0', STR_PAD_LEFT);

  // Collect form data
  $title = $_POST['title'] ?? '';
  $description = $_POST['description'] ?? '';
  $date_started = $_POST['dateStarted'] ?? date('Y-m-d');
  $status = $_POST['status'] ?? 'Ongoing';
  $priority = $_POST['priority'] ?? 'Medium';
  $assigned_officer_id = $_POST['officer'] ?? NULL; // This is now the officer ID, not name
  $evidence = $_POST['evidence'] ?? '';
  $progress_percent = 0;

  // Insert case into database
  $sql = "INSERT INTO `case` 
            (case_id, title, description, date_started, status, priority, assigned_officer_id, progress_percent) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

  $stmt = $pdo->prepare($sql);

  if ($stmt->execute([$case_id, $title, $description, $date_started, $status, $priority, $assigned_officer_id, $progress_percent])) {
    error_log("Case inserted: ID=$case_id, Title=$title, Officer=$assigned_officer_id");
    header("Location: cases.php?msg=added");
    exit;
  } else {
    die("Error saving case: " . implode(" ", $stmt->errorInfo()));
  }
}
...
<body>
  ...
  <div class="container" style="max-width: 900px;">
    <!-- Header -->
    <div class="mb-4">
      <button type="button" class="btn btn-outline-link p-0 mb-3" onclick="window.location.href='cases.php'">
      ...
      <h1>Add New Case</h1>
      <p class="text-muted">Create a new investigation case record in the system</p>
    </div>

    <!-- Success alert placeholder -->
    <div id="successAlert" class="alert alert-success d-none" role="alert">
      Case created successfully! Redirecting to cases list...
    </div>

    <!-- Form -->
    <form id="caseForm" method="POST" action="newcases.php" novalidate>
      <input type="hidden" name="evidence" id="evidenceHiddenInput">
      <!-- Case Information Card -->
      <div class="card mb-4">
        <div class="card-header">
          <h5 class="card-title mb-0">Case Information</h5>
          <small class="text-muted">Enter the basic details about the investigation case</small>
        </div>
        <div class="card-body">
          <div class="mb-3">
            <label for="title" class="form-label">Case Title <span class="text-danger">*</span></label>
            <input type="text" class="form-control" id="title" name="title" placeholder="e.g., Commercial Burglary Investigation" required>
            <div class="invalid-feedback">Case Title is required.</div>
          </div>

          <div class="mb-3">
            <label for="description" class="form-label">Case Description <span class="text-danger">*</span></label>
            <textarea class="form-control" id="description" name="description" rows="4" placeholder="Provide detailed description of the case..." required></textarea>
            <div class="invalid-feedback">Case Description is required.</div>
          </div>

          <div class="mb-3">
            <label for="dateStarted" class="form-label">Date Started</label>
            <input type="date" class="form-control" id="dateStarted" name="dateStarted" value="" required>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-4 mb-3">
              <label for="officerSelect" class="form-label">Assigned Officer <span class="text-danger">*</span></label>
              <select class="form-select" id="officerSelect" name="officer" required>
              ...
              <div class="invalid-feedback">Assigned Officer is required.</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card mb-4">
        <div class="card-header">
          ...
          <small class="text-muted">Add evidence items associated with this case</small>
        </div>
        <div class="card-body">
          <div class="input-group mb-3">
            <input type="text" class="form-control" id="evidenceInput" placeholder="e.g., Security camera footage, Fingerprint samples" aria-label="Evidence item" />
            <button class="btn btn-outline-primary" type="button" id="addEvidenceBtn">Add</button>
          </div>

          <div id="evidenceList" class="d-flex flex-wrap gap-2">
            <p class="text-muted">No evidence items added yet. Add evidence using the input above.</p>
          </div>
        </div>
      </div>
      ...
      <div class="d-flex gap-3">
        ...
        <button type="button" class="btn btn-outline-secondary flex-grow-1" onclick="window.location.href='cases.php'">Cancel</button>
      </div>
    </form>
  </div>
  ...
  <script>
    ...
    function renderEvidence() {
      ...
      if (evidenceItems.length === 0) {
        ...
        p.textContent = 'No evidence items added yet. Add evidence using the input above.';
        evidenceListDiv.appendChild(p);
        return;
      }
      evidenceItems.forEach((item, index) => {
        const badge = document.createElement('span');
        badge.className = 'badge bg-secondary d-flex align-items-center gap-2 me-2 mb-2';
        ...
        evidenceListDiv.appendChild(badge);
      });
    }

    addEvidenceBtn.addEventListener('click', () => {
      const val = evidenceInput.value.trim();
      if (val) {
        evidenceItems.push(val);
        evidenceInput.value = '';
        renderEvidence();
      }
    });

    // Allow Enter key to add evidence
    ...
    const evidenceHiddenInput = document.getElementById('evidenceHiddenInput');
    ...
    form.addEventListener('submit', (e) => {
      ...
      // Put evidence array into hidden input so PHP can read it
      evidenceHiddenInput.value = evidenceItems.join(', ');
