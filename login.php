...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($username === '' || $password === '') {
    ...
    // 1. Select the password and status as well
    $stmt = $pdo->prepare("
    ...
    // 2. Verify the password hash (CRITICAL SECURITY STEP)
    elseif (!password_verify($password, $user['password'])) {
      $error = 'Invalid username or password.';
    }
    // 3. Fixed the typo from 'accountphp_status' to 'account_status'
    ...
    <div class="card mx-auto" style="max-width: 410px;">
      <div class="card-body">
        ...
        <div class="mt-2 text-center small">
          ...
          <a href="/signup.php" class="text-primary text-decoration-underline fw-semibold">Request Access</a>
        </div>
      </div>
    </div>
  </div>
